...
/* QUESTAO 2 */
SELECT nome AS professores_de_Administração
FROM professores
WHERE disciplina = 'Administracão';
...
/* QUESTAO 4 */
SELECT curso, COUNT(curso) AS total_cursos
FROM alunos
GROUP BY curso;

/* QUESTAO 5 */
SELECT curso, COUNT(curso) AS total
FROM alunos
GROUP BY curso
HAVING COUNT(*) > 6;
...
/* QUESTAO 7 */
SELECT a.nome AS alunos, t.nome_turma 
FROM alunos a 
...
/* QUESTAO 11 */
SELECT AVG(idade) AS media_idade_contabilidade
FROM alunos
